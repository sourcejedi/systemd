...
/* Check authorization to schedule a shutdown in the future,
 * when we don't know what inhibiting factors will be present. */
static int verify_all_shutdown_creds(
                Manager *m,
...
/* Check authorization to shutdown, considering current inhibiting factors. */
static int verify_shutdown_creds(
        ...
        if (r < 0)
                return r;
        if (r == 0)
                action_multiple_sessions = NULL;

        if (!manager_is_inhibited(m, w, INHIBIT_BLOCK, NULL, false, true, uid, NULL))
                action_ignore_inhibit = NULL;

        return verify_all_shutdown_creds(m, message, interactive, action,
                                         action_multiple_sessions, action_ignore_inhibit,
                                         error);
}
